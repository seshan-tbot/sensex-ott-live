...
async function loadHistoryDays() {
  // 2026-09-10 fix: this used to run once per page session (a
  // `recapDaysLoaded` guard) and fetch without cache-busting -- combined
  // with GitHub Pages' default 10-min CDN cache on all published files
  // (no custom response headers possible on Pages, so this has to be
  // fought client-side), a page left open across today's first trade, or
  // just reloaded within that 10-min window, could permanently miss
  // today's entry in the Recap day list for the rest of the session while
  // the local dashboard (no caching at all) already showed it. Re-fetching
  // fresh every time this tab is opened matches how pollState() already
  // treats data/state.json below.
  let d;
  try {
    d = await (await fetch("data/history_days.json?_=" + Date.now())).json();
  } catch {
    return;
  }
  ...
  // Keep whatever day the user had selected (e.g. they're reviewing an
  // older day) across this refresh instead of always snapping back to the
  // most recent one -- only default to days[0] the first time, or if their
  // prior selection somehow no longer exists.
  const prevSelected = select.value;
  select.innerHTML = days.map((day) => `<option value="${day}">${day}</option>`).join("");
  const target = days.includes(prevSelected) ? prevSelected : days[0];
  select.value = target;
  loadHistoryDay(target);
}
...
async function loadHistoryDay(date) {
  ...
  let d;
  try {
    const r = await fetch("data/history/" + encodeURIComponent(date) + ".json?_=" + Date.now());
    if (!r.ok) return;
    d = await r.json();
  } catch {
    return;
